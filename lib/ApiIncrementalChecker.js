...
class ApiIncrementalChecker {
    constructor(typescript, createNormalizedMessageFromDiagnostic, createNormalizedMessageFromRuleFailure, programConfigFile, compilerOptions, context, linterConfigFile, linterAutoFix, checkSyntacticErrors, enableEmitFiles = false, resolveModuleName, resolveTypeReferenceDirective) {
        ...
        this.tsIncrementalCompiler = new CompilerHost_1.CompilerHost(typescript, programConfigFile, compilerOptions, checkSyntacticErrors, enableEmitFiles, resolveModuleName, resolveTypeReferenceDirective);
    }
    initLinterConfig() {
    ...
    emitFiles() {
        try {
            ...
            if (!program) {
                throw new Error('Program (a ts.Program object) in emitFiles() for ApiIncrementalChecker.ts not found');
            }
